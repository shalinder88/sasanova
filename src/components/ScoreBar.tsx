export default function ScoreBar({ label, value, max = 10 }: { label: string; value: number; max?: number }) {
  ...
  return (
    <div className="hover-score-row flex items-center gap-2 text-xs group">
      ...
      <span className={`w-6 text-right font-mono font-medium group-hover:text-sm transition-all ${value >= 8 ? "group-hover:text-success" : value >= 5 ? "group-hover:text-accent" : "group-hover:text-warning"}`}>{value}</span>
    </div>
  );
}
