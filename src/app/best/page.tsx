...
const iconMap: Record<string, React.ReactNode> = {
  ...
  zap: <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" /></svg>,
  calendar: <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6.75 3v2.25M17.25 3v2.25M3 18.75V7.5a2.25 2.25 0 012.25-2.25h13.5A2.25 2.25 0 0121 7.5v11.25m-18 0A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75m-18 0v-7.5A2.25 2.25 0 015.25 9h13.5A2.25 2.25 0 0121 11.25v7.5" /></svg>,
  ...
  send: <svg className="w-6 h-6" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" /></svg>,
    ...
    <>
      <section className="border-b border-border bg-surface">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <p className="text-xs font-semibold uppercase tracking-wider text-accent mb-3">Best Of</p>
            ...
            {categories.filter((cat) => getToolsByCategory(cat.slug).length >= 2).map((cat) => {
              ...
              return (
                <Link
                  key={cat.slug}
                  href={`/best/${cat.slug}`}
                  className="group flex items-start gap-4 p-5 border border-border rounded-xl bg-background hover:border-accent/30 hover:bg-accent-light/50 transition-all"
                >
                  <div className="text-accent shrink-0 mt-0.5">
                    {iconMap[cat.icon] || iconMap.layout}
                  </div>
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-accent font-medium mt-2">{toolCount} tools ranked →</p>
                  </div>
                </Link>
              );
            })}
